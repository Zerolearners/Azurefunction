...
{
    public class DiamondMemberShipSecondQuarter
    {
        ...
        public DiamondMemberShipSecondQuarter(ILoggerFactory loggerFactory, FinderSchedulerDbContext context, FinderSetting setting, IMatches matches, MailSetting mailSetting)
        {
            _logger = loggerFactory.CreateLogger<DiamondMemberShipSecondQuarter>();
            _context = context;
            _setting = setting;
            _mailsetting = mailSetting;
            this.matches = matches;
        }
        [Function("DiamondMemberShipSecondQuarter")]// 2 am will start
        public async Task Run([TimerTrigger("30 20 * * *")] TimerInfo myTimer)
        {
            _logger.LogInformation($"DiamondMemberShipSecondQuarter function executed at: {DateTime.Now}");
            ...
            if (myTimer.ScheduleStatus is not null)
            {
                try
                {
                    ...
                    string[] membershipCodes = new string[] { CommonConstant.DiamondCode };
                    var candidateDetailsForEmail = await matches.SendPushData(membershipCodes, 2);
