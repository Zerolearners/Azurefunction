...
{
    public class DiamondMemberShipThirdQuarter
    {
        ...
        public DiamondMemberShipThirdQuarter(ILoggerFactory loggerFactory, FinderSchedulerDbContext context, FinderSetting setting, IMatches matches, MailSetting mailSetting)
        {
            _logger = loggerFactory.CreateLogger<DiamondMemberShipThirdQuarter>();
            _context = context;
            _setting = setting;
            _mailsetting = mailSetting;
            this.matches = matches;
        }
        [Function("DiamondMemberShipThirdQuarter")]// 2 am will start
        public async Task Run([TimerTrigger("30 20 * * *")] TimerInfo myTimer)
        {
            _logger.LogInformation($"DiamondMemberShipThirdQuarter function executed at: {DateTime.Now}");
            ...
            if (myTimer.ScheduleStatus is not null)
            {
                try
                {
                    ...
                    var candidateDetailsForEmail = await matches.SendPushData(membershipCodes, 3);
