...
{
    public class DiamondMemberShipFirstQuarter
    {
        ...
        public DiamondMemberShipFirstQuarter(ILoggerFactory loggerFactory, FinderSchedulerDbContext context, FinderSetting setting, IMatches matches, MailSetting mailSetting)
        {
            _logger = loggerFactory.CreateLogger<DiamondMemberShipFirstQuarter>();
            _context = context;
            _setting = setting;
            _mailsetting = mailSetting;
            this.matches = matches;
        }
        [Function("DiamondMemberShipFirstQuarter")]// 2 am will start
        public async Task Run([TimerTrigger("30 20 * * *")] TimerInfo myTimer)
        {
            _logger.LogInformation($"DiamondMemberShipFirstQuarter function executed at: {DateTime.Now}");
            ...
            if (myTimer.ScheduleStatus is not null)
            {
                try
                {
                    ...
                    var candidateDetailsForEmail = await matches.SendPushData(membershipCodes, 1);
