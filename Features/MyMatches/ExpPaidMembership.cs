...
{
    public class ExpPaidMembership
    {
        ...
        public ExpPaidMembership(ILoggerFactory loggerFactory, FinderSchedulerDbContext context, FinderSetting setting, IMatches matches, MailSetting mailSetting)
        {
            _logger = loggerFactory.CreateLogger<ExpPaidMembership>();
            _context = context;
            _setting = setting;
            _mailsetting = mailSetting;
            this.matches = matches;
        }
        [Function("ExpPaidMembership")]//2 am will start 
        public async Task Run([TimerTrigger("30 20 * * *")] TimerInfo myTimer)
        {
            _logger.LogInformation($"ExpPaidMembership function executed at: {DateTime.Now}");
            ...
            if (myTimer.ScheduleStatus is not null)
            {
                try
                {
                    ...
                    var membership = await _context.subscriptions.Include(e => e.SubscriptionCategory).Where(e => e.SubscriptionCategory.Code == CommonConstant.ExpiredPremiumMembers && e.IsActive).ToListAsync();
