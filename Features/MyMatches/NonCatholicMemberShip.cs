...
{
    public class NonCatholicMemberShip
    {
        ...
        public NonCatholicMemberShip(ILoggerFactory loggerFactory, FinderSchedulerDbContext context, FinderSetting setting, IMatches matches, MailSetting mailSetting)
        {
            _logger = loggerFactory.CreateLogger<NonCatholicMemberShip>();
            _context = context;
            _setting = setting;
            _mailsetting = mailSetting;
            this.matches = matches;
        }
        [Function("NonCatholicMemberShip")]// 2 am will start
        public async Task Run([TimerTrigger("30 20 * * *")] TimerInfo myTimer)
        {
            _logger.LogInformation($"NonCatholicMemberShip function executed at: {DateTime.Now}");
            ...
            if (myTimer.ScheduleStatus is not null)
            {
                try
                {
                    ...
                    string inputCode = CommonConstant.NonCatholicCode;
                    string[] membershipCodes = inputCode.Split(',');
